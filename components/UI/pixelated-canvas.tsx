...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
  ...
  React.useEffect(() => {
    ...
    const img = new Image() as CustomImage;
    ...
    const compute = () => {
      ...
      type TTintRGB = [number, number, number] | null;

      let tintRGB: TTintRGB = null;
      if (tintColor && tintStrength > 0) {
        const parse = (c: string): TTintRGB => {
          if (c.startsWith("#")) {
            const hex = c.slice(1);
            if (hex.length === 3) {
        ...
        tintRGB = parse(tintColor) as TTintRGB;
      }
