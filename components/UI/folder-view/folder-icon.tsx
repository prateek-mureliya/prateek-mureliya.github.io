    ...
    <PinContainer icon={icon} title={linkTitle} href={link}>
      <div className="border flex flex-col rounded-lg h-full overflow-hidden group">
        <h1 className="flex items-center text-[10px] sm:text-xs p-2 group-hover:invisible">
          <Image alt={linkTitle} src={icon} placeholder="blur" className="mr-1 mt-[1px] size-3" />
          <span>{linkTitle}</span>
        </h1>
        <div className="flex-1 overflow-hidden">
          <Image alt={linkTitle!} src={viewer!} placeholder="blur" className="w-full h-full" />
        </div>
      </div>
    </PinContainer>
  );
  ...
  return (
    ...
    >
      <figure className="flex flex-col place-items-center">
        {(type == 'window' || type == 'raw') && <WindowIconCard id={id} title={title} icon={icon} viewer={viewer} />}
        {type == 'link' && <LinkIconCard {...(others as TProcessButtonLink)} icon={icon} viewer={viewer} />}
        <figcaption className="text-xs text-shadow-xs text-center mt-1 wrap-anywhere select-none">{title}</figcaption>
      </figure>
    </div>
  );
}
