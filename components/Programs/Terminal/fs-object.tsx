...
import { FaFacebook, FaGithub, FaInstagram, FaLinkedin, FaXTwitter, FaRegUser, FaLink, FaImage } from 'react-icons/fa6';
...
const $social: TFolder = {
  ...
  createdAt: 'Aug 31, 4:58 AM GMT+5:30',
  name: 'social',
  dir: [$facebook_connect, $github_connect, $instagram_connect, $linkedin_connect, $twitter_connect],
};
...
export const getFiles = (path: string[], reletivePaths: string[]): TFile[] => {
  return reletivePaths.map((reletivePath) => getFile(getAbsolutePath(path, reletivePath)));
};

export const getDirectories = (path: string[], reletivePaths: string[]): TFolder[] => {
  return reletivePaths.map((reletivePath) => getDirectory(getAbsolutePath(path, reletivePath)));
};
